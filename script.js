...
(function () {
  ...
  if (btn) {
    btn.addEventListener('click', function () {
      ...
      syncIcons();
      // Reset chart flag so charts rebuild with updated theme colours
      chartsBuilt = false;
    });
  }
})();
...
function fmtDate(iso) {
  if (!iso) return '';
  var d = new Date(iso);
  if (isNaN(d)) return iso; // guard against Invalid Date
...
function getChartPalette(count) {
  ...
  var base = ['--chart-1','--chart-2','--chart-3','--chart-4','--chart-5','--chart-6','--chart-7','--chart-8']
    .map(function (v) { return root.getPropertyValue(v).trim(); })
    .filter(Boolean);

  var colors = base.slice();
...
function buildCharts(workflows) {
  ...
  var durationLabels = [], durationValues = [];
...
function lastRunAge(wf) {
  if (!wf.last_run) return Infinity;
  return Date.now() - new Date(wf.last_run.run_started_at || wf.last_run.created_at).getTime();
}

function ranWithin24h(wf)  { return lastRunAge(wf) < MS_24H; }
function hasRecentRun(wf)  { return lastRunAge(wf) < MS_7_DAYS; }

function isRecentFailure(wf) {
  return (wf.recent_runs || []).some(function (run) {
    if (run.conclusion !== 'failure') return false;
    return (Date.now() - new Date(run.run_started_at || run.created_at).getTime()) < MS_7_DAYS;
  });
}
...
function sortList(list) {
  switch (activeFilter) {
    case 'recent-run':
    case 'recent-failure':
      // Both sort by most-recent run first
      return list.slice().sort(function (a, b) {
        return lastRunAge(a) - lastRunAge(b);
      });
    case 'failing':
    ...
    tr.innerHTML =
      '<td>'
        + '<div class="wf-name-wrap">'
          + '<svg class="chevron" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><path d="M6.22 3.22a.75.75 0 0 1 1.06 0l4.25 4.25a.75.75 0 0 1 0 1.06l-4.25 4.25a.749.749 0 0 1-1.06-1.06L10 8 6.22 4.22a.75.75 0 0 1 0-1Z"/></svg>'
          + '<span class="wf-name">'
            + esc(wf.name)
      ...
      + '<td>'
        + (lr
          ? conclusionHtml(lr.conclusion) + '<div class="run-date">#' + lr.run_number + ' · ' + fmtDate(lr.run_started_at || lr.created_at) + '</div>'
      ...
      + '<td>'
        + (lr
          ? '<a class="run-link" href="' + esc(lr.html_url) + '" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()">↗ View</a>'
...
function activateFilter(filter) {
  ...
  if (filter === 'chart') {
    ...
    chartsPanel.setAttribute('aria-hidden', 'false');
    ...
    chartsPanel.setAttribute('aria-hidden', 'true');
...
document.querySelectorAll('.chip').forEach(function (chip) {
  chip.addEventListener('click', function () { activateFilter(chip.dataset.filter); });
});
...
function renderDrawer(wf, drawerEl) {
  ...
  runs.forEach(function (run, ri) {
    ...
    html += '<tr>'
      ...
      + '<td><a class="run-link" href="' + esc(run.html_url) + '" target="_blank" rel="noopener noreferrer">↗ View</a></td>'
      ...
      run.failed_jobs.forEach(function (job) {
        html += '<div class="log-job-block">'
          + '<div class="log-job-header">'
            ...
            + '<a class="log-job-link" href="' + esc(job.html_url) + '" target="_blank" rel="noopener noreferrer">Open in GitHub ↗</a>'
          + '</div>'
  ...
  setTimeout(function () {
    ...
    new Chart(document.getElementById(passfailId), {
      ...
      options: {
        ...
        tooltips: {
          callbacks: {
            label: function (item) { return ' ' + sorted[item.index].conclusion + ' · ' + item.yLabel + ' min'; }
          }
        }
      }
    });
  ...
  drawerEl.querySelectorAll('.drawer-tab').forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      if (target === 'charts') {
        document.querySelectorAll('#panel-charts-' + safeWfName + ' .wf-chart-block').forEach(function (block, i) {
...
fetch('stats.json')
  ...
  .then(function (data) {
    ...
    if (generatedAt && !isNaN(generatedAt) && lastUpdatedEl) {
    ...
    /* 24-hour stat cards */
    var windowStart = new Date(Date.now() - MS_24H);
    ...
    allWorkflows.forEach(function (wf) {
      (wf.recent_runs || []).forEach(function (run) {
        ...
        else if (run.conclusion === 'failure' || run.conclusion === 'timed_out') todayFail++;
      });
    });
  ...
  .catch(function (e) {
    document.getElementById('wfBody').innerHTML =
      '<tr class="loading-row"><td colspan="6">Error loading stats.json: ' + esc(String(e)) + '</td></tr>';
  });
/* ─── Theme ──────────────────────────────────────────────────────────────── */
(function () {
  var saved = localStorage.getItem('ci-theme') || 'dark';
...
function buildCharts(workflows) {
  ...
  // v4: set global defaults once
  Chart.defaults.font.family = "'JetBrains Mono', monospace";
  Chart.defaults.font.size   = 11;
  Chart.defaults.color       = mutedColor();

  // Shared scale defaults used across charts
  function scaleBase() {
    return {
  ...
  new Chart(document.getElementById('chartPie'), {
    ...
    options: {
      ...
      maintainAspectRatio: true,
      plugins: {
        legend: { position: 'right', labels: { boxWidth: 12, font: { size: 11 }, color: mutedColor() } },
        tooltip: {
          callbacks: {
            label: function (ctx) {
              var w = active[ctx.dataIndex];
              return ' ' + ctx.label
                + ': ' + (w.failure_rate > 20 ? 'failing' : 'passing')
                + ' · ' + (100 - w.failure_rate).toFixed(1) + '% success'
                + ' · ' + w.total_runs + ' runs';
  ...
  var lineDatasets = active.map(function (w, wi) {
    ...
    return {
      ...
      fill: false,
      tension: 0.3   // v4: lineTension → tension
    };
  });
  ...
  var slotLabels = [];
  for (var i = maxSlots; i >= 1; i--) slotLabels.push(i === 1 ? 'latest' : String(i));
  ...
  new Chart(document.getElementById('chartBar'), {
    ...
    options: {
      ...
      maintainAspectRatio: true,
      plugins: {
        legend: { labels: { boxWidth: 12, font: { size: 11 }, color: mutedColor() } },
        tooltip: {
          mode: 'index',
          intersect: false,
          callbacks: {
...
function renderDrawer(wf, drawerEl) {
  ...
  setTimeout(function () {
    ...
    // v4 global defaults (safe to re-set per drawer)
    Chart.defaults.font.family = "'JetBrains Mono', monospace";
    Chart.defaults.font.size   = 11;
    Chart.defaults.color       = mutedColor();
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                label: function (ctx) {
                  var count = ctx.dataset._counts[ctx.dataIndex];
                  return ' ' + ctx.dataset.label + ': ' + ctx.parsed.y + 's (avg of ' + count + ' run' + (count === 1 ? '' : 's') + ')';
                }
              }
            }
          },
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: function (ctx) {
                return ' ' + sorted[ctx.dataIndex].conclusion + ' · ' + ctx.parsed.y + ' min';
              }
            }
          }
        },
